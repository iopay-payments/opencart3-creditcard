...
/**
 * Class ControllerExtensionPaymentIOPAYCheckoutTransparente
 */
class ControllerExtensionPaymentIOPAYCheckoutTransparenteCard extends Controller{
    ...
    public function index(){
        ...
        unset($this -> session -> data['iopay_request_fails']);
        unset($this -> session -> data['iopay_sec_token']);
        ...
        $data['testmode'] = $this->config->get('payment_iopay_checkout_transparente_card_test');

        if (!$this->config->get('payment_iopay_checkout_transparente_card_test')) {
            $data['action'] = 'https://checkout.iopay.com.br/app/pay/' . $this->config->get('payment_iopay_checkout_transparente_card_ioid');
        } else {
            $data['action'] = 'https://sandbox.checkout.iopay.com.br/app/pay/' . $this->config->get('payment_iopay_checkout_transparente_card_ioid');
        }
        ...
        $customer_primarydocument_id =  $this->config->get('payment_iopay_checkout_transparente_card_mapping_user_primarydocument');
        $shipping_number_shipping_id =  $this->config->get('payment_iopay_checkout_transparente_card_mapping_shipping_addressnumber');
        ...
        if($custom_customer_primarydocument == '' || $custom_shipping_phonenumber == '' || $custom_shipping_primarydocument == ''){
        ...
        if ($order_info) {
            ...
            $data['business'] = $this->config->get('payment_iopay_checkout_transparente_card_email');
            ...
            $data['notify_url'] = $this->url->link('extension/payment/iopay_checkout_transparente_card/callback', '', true);
            ...
            // IO Seller ID
            $data['iopay_seller_id'] = $this->config->get('payment_iopay_checkout_transparente_card_ioid');
            ...
            $data['iopay_antifraud_id'] = $this->config->get('payment_iopay_checkout_transparente_card_antifraud_id');
            $data['iopay_antifraud_type'] = $this->config->get('payment_iopay_checkout_transparente_card_antifraud_type');
            ...
            if (!$this->config->get('payment_iopay_checkout_transparente_card_transaction')) {
            ...
            for($i = 1; $i<= $data['payment_iopay_checkout_transparente_card_installment_max']; $i++){
                $installment = $this->installmentsCalculator($data['amount'], $i);
                $data['iopay_installments_simulator'][] = $installment;
            }

            $data['iopay_sec_token'] = $this -> getToken();
            return $this->load->view('extension/payment/iopay_checkout_transparente_card', $data);
        }
    }
    ...
    private function addRequestFail(){
        if(!isset($this->session->data['iopay_request_fails'])){
            $this->session->data['iopay_request_fails'] = 1;
        }else{
            $this->session->data['iopay_request_fails'] = $this->session->data['iopay_request_fails'] + 1;
        }
        return $this->session->data['iopay_request_fails'];
    }

    private function installmentsCalculator($amount, $installment_number):array {
        $return = array();
        $value = round((1 + ((int)$this->config->get('payment_iopay_checkout_transparente_card_installment_fee_'.$installment_number.'x'))/100) * $amount, 2);

        if($installment_number == 1){
            $return['label'] = "Crédito à vista (R$ ". str_replace(".",",",round(($value/$installment_number), 2)).")";
            $return['amount'] = $value;
            return $return;
        }else {
            $return['label'] = "Parcelado em ".$installment_number."x de R$ ". str_replace(".",",",round(($value/$installment_number), 2))." (Total R$ ".$value.")";
            $return['amount'] = $value;
            return $return;
        }
    }
    ...
    public function send() {
       ...
       ini_set('display_errors', 0);
        ...
        if ($order_info) {
            ...
            $custom_customer_primarydocument = null;
            $custom_shipping_number = 0;
            ...
            // payment type
            if($this->request->post['payment_type'] == 'credit'){
                $payment_type = 'credit';
            }else {
                $this->log->write('IOPAY - Erro, tipo de transação invalida. Abortando...');
                exit();
            }
            ...
            if (isset($customer_response->error)) {
                ...
                $json['iopay_sec_token'] = $this -> getToken();
                    ...
                    // Recupera o amount de acordo com o acréscimo configurado no parcelamento via admin
                    $installments_config = $this -> installmentsCalculator($order_info['total'], $this->request->post['installment_plan']['number_installments']);
                    $amount = $installments_config['amount'] * 100;

                    // 2. enviamos a transação fornecendo o id do comprador e id do cartão tokenizado ou boleto
                    $transactionParams = array(
                        "amount" => (int)$amount,  // valor processado precisa estar em centavos, então multiplicamos por 100
                        "currency" => 'BRL',
                        "description" => "Compra online: " . $order_info['store_name'],
                        "token" => utf8_encode($this->request->post['token']),
                        "capture" => $this->config->get('payment_iopay_checkout_transparente_card_transaction'),
                        "statement_descriptor" => $order_info['store_name'] . " - ID: " . $order_info['order_id'],
                        "installment_plan" => array("number_installments" => $this->request->post['installment_plan']['number_installments']),
                        "io_seller_id" => $this->config->get('payment_iopay_checkout_transparente_card_ioid'),
                        "payment_type" => $payment_type,
                        "reference_id" => $order_info['order_id'],
                        'antifraud_sessid' => sha1($order_info['order_id']),
                    );

                // Monta os parametros de dados de entrega
                if (isset($custom_shipping_phonenumber) && $custom_shipping_phonenumber != '' && isset($custom_shipping_primarydocument) && $custom_shipping_primarydocument != '') {

                    $shipping_taxpayer_id = $custom_shipping_primarydocument;
                    $shipping_phone_number = $custom_shipping_phonenumber;

                    /*
                    if($order_info['shipping_address_2'] == null) {
                        if (preg_match('/([^\d]+)\s?(.+)/i', $order_info['shipping_address_1'], $resultAddress)) {
                            $streetName = $resultAddress[1];
                            $streetNumber = $resultAddress[2];
                        }
                    }else {
                        $streetName = $order_info['shipping_address_1'];
                        $streetNumber = $order_info['shipping_address_2'];
                    }
                    */

                    $transactionParams['shipping']['taxpayer_id'] = $shipping_taxpayer_id;
                    $transactionParams['shipping']['firstname'] = $order_info['shipping_firstname'];
                    $transactionParams['shipping']['lastname'] = $order_info['shipping_lastname'];
                    $transactionParams['shipping']['address_1'] = $streetName;
                    $transactionParams['shipping']['address_2'] = $streetNumber;
                    $transactionParams['shipping']['address_3'] = $order_info['shipping_address_2'];
                    $transactionParams['shipping']['postal_code'] = $order_info['shipping_postcode'];
                    $transactionParams['shipping']['city'] = $order_info['shipping_city'];
                    $transactionParams['shipping']['state'] = $order_info['shipping_zone_code'];
                    $transactionParams['shipping']['client_type'] = "pf";
                    $transactionParams['shipping']['phone_number'] = $shipping_phone_number;
                }
                ...
                if(isset($transaction_response->success)){
                    $json = $this -> addSucceededTransaction($transaction_response, $order_info['order_id'],$this->model_checkout_order, $installments_config);
                }else if(isset($transaction_response->error)){
                    $json = $this -> addFailedTransaction($transaction_response, $order_info['order_id'],$this->model_checkout_order, $installments_config);
                }else {
    ...
    function addSucceededTransaction($transaction_response, $order_id, $model_checkout_order, $installments_config):array {
        ...
        if (isset($transaction_response->success->payment_method)) {
            $message .= '<i class="fa fa-user-o" aria-hidden="true"></i> &nbsp;<strong>Portador do cart&atilde;o: </strong><br>' . $transaction_response->success->payment_method->holder_name . "<br><br>";
            $message .= '<i class="fa fa-credit-card" aria-hidden="true"></i> &nbsp;<strong>Cart&atilde;o: </strong><br>' . $transaction_response->success->payment_method->first4_digits . ' **** **** ' . $transaction_response->success->payment_method->last4_digits . " &nbsp;&nbsp;&nbsp; (" . ($transaction_response->success->payment_method->card_brand) . ")<br><br>";
            $message .= '<i class="fa fa-clock-o" aria-hidden="true"></i> &nbsp;<strong>Validade: </strong><br>' . $transaction_response->success->payment_method->expiration_month . "/" . $transaction_response->success->payment_method->expiration_year . "<br><br>";
        }

        if (isset($transaction_response->success->installment_plan)) {
            $message .= '<i class="fa fa-check-circle-o" aria-hidden="true"></i> &nbsp;<strong>Parcelamento</strong><br>';
            $message .= $installments_config['label'].'<br><br>';
        }

        if (isset($transaction_response->success->sales_receipt)) {
            $message .= '<i class="fa fa-check-circle-o" aria-hidden="true"></i> &nbsp;<strong>N&ordm; do Comprovante:</strong><br>' . $transaction_response->success->sales_receipt . "<br><br>";
        }

        if (isset($transaction_response->success->antifraud_status)) {
            $message .= '<i class="fa fa-shield" aria-hidden="true"></i> &nbsp;<strong>Antifraude:</strong><br>' . $transaction_response->success->antifraud_status . "<br><br>";
        }
        ...
        if (isset($transaction_response->success->captured)) {
            $message .= ($transaction_response->success->captured == '1' ? ' <span class="label label-success">PAGAMENTO CONFIRMADO' : ' <span class="label label-warning">EM ANÁLISE ANTIFRAUDE') . "</span><br>";
        }

        if (isset($transaction_response->success->amount)) {
            $message .= '<h3>R$' . $transaction_response->success->amount . "</h3>";
        }

        if ($transaction_response->success->status == 'pre_authorized') {

            $status = $this->config->get('payment_iopay_checkout_transparente_card_pending_status_id');
            ...
            $status = $this->config->get('payment_iopay_checkout_transparente_card_processed_status_id');
            $model_checkout_order->addOrderHistory($order_id, $status, $message, true, true);
        ...
        $installments = $this -> installmentsCalculator($order_info['total'], $transaction_response->success->installment_plan->number_installments);

        $this->session->data['iopay_transaction_id'] = $transaction_response->success->id;
        $this->session->data['iopay_ecommerce_order_id'] = $order_id;
        $this->session->data['iopay_sandbox_mode'] = $this->config->get('payment_iopay_checkout_transparente_card_test');
        $this->session->data['iopay_total_order'] = $transaction_response->success->amount;
        $this->session->data['iopay_payment_type'] = 'card';
        $this->session->data['iopay_installment_label'] = $installments['label'];
    ...
    function addFailedTransaction($transaction_response, $order_id, $model_checkout_order, $installments_config):array{

        $order_info = $model_checkout_order->getOrder($order_id);

        if(isset($transaction_response->error->message)){
            $msg = $transaction_response->error->message. "(OrderID: ".$order_info['order_id'].")";
            $this->log->write('IOPAY - Falha ao gerar transação de pagamento: ' . $msg . ')');
        }
        ...
        if (isset($transaction_response->error->created_at)) {
            $date_generated = date_create($transaction_response->error->created_at);
            $message .= '<i class="fa fa-calendar-check-o" aria-hidden="true"></i> &nbsp;<strong>Data de geração:</strong><br>' . date_format($date_generated, "d/m/Y H:i:s") . "<br><br>";
        }

        if (isset($transaction_response->error->id)) {
            $message .= '<i class="fa fa-check-circle-o" aria-hidden="true"></i> &nbsp;<strong>ID da Transa&ccedil;&atilde;o:</strong><br>' . $transaction_response->error->id . "<br><br>";
        }

        if (isset($transaction_response->error->payment_method)) {
            $message .= '<i class="fa fa-user-o" aria-hidden="true"></i> &nbsp;<strong>Portador do cart&atilde;o: </strong><br>' . $transaction_response->error->payment_method->holder_name . "<br><br>";
            $message .= '<i class="fa fa-credit-card" aria-hidden="true"></i> &nbsp;<strong>Cart&atilde;o: </strong><br>' . $transaction_response->error->payment_method->first4_digits . ' **** **** ' . $transaction_response->error->payment_method->last4_digits . " &nbsp;&nbsp;&nbsp; (" . ($transaction_response->error->payment_method->card_brand) . ")<br><br>";
            $message .= '<i class="fa fa-clock-o" aria-hidden="true"></i> &nbsp;<strong>Validade: </strong><br>' . $transaction_response->error->payment_method->expiration_month . "/" . $transaction_response->error->payment_method->expiration_year . "<br><br>";
        }

        if (isset($transaction_response->error->installment_plan)) {
            $message .= '<i class="fa fa-check-circle-o" aria-hidden="true"></i> &nbsp;<strong>Parcelamento</strong><br>';
            $message .= $installments_config['label'].'<br><br>';
        }

        if (isset($transaction_response->error->antifraud_status)) {
            $message .= '<i class="fa fa-shield" aria-hidden="true"></i> &nbsp;<strong>Antifraude:</strong><br>' . $transaction_response->error->antifraud_status . "<br><br>";
        }

        if (isset($transaction_response->error->amount)) {
    ...
    public function callback(){
        ...
        if ($order_info) {
            ...
            if($response -> success -> status == 'canceled' && ($response->success->captured != '1' || $response->success->voided != '1')){
                $label = "<span style='color:darkred;font-weight: bold'>NÃO APROVADO</span>";
                $order_status_id = $this->config->get('payment_iopay_checkout_transparente_card_failed_status_id');
            }
